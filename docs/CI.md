...
- **lint** — `npm run check:lint` (ESLint + Prettier `--check`)
- **locale** — prescriptive `--check` for EN/SV + tier validators
- **test** — Vitest
...
Runs `npm run sv:pipeline` and `npm run en:pipeline` in parallel (no `--with-lm`, no Wikipedia extended). Artifacts retain pipeline logs and `PIPELINE-SNAPSHOT.md` for 14 days. On failure, the workflow opens a GitHub issue with a link to the run.
