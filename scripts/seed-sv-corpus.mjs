 ...
 *
 * Run from repo root: node scripts/seed-sv-corpus.mjs
 */
...
const outHuman = path.join(root, 'tests/fixtures/sv-corpus/human');
const outAi = path.join(root, 'tests/fixtures/sv-corpus/ai');
...
function main() {
  ...
  console.log(`Wrote 50 files to ${path.relative(root, outHuman)}`);
  console.log(`Wrote 50 files to ${path.relative(root, outAi)}`);
}
