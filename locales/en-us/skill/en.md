# Humanizer — English locale (`en`, `en-us`)

Same CLI locale code as `en-en` (`en`); this copy lives under the **en-us** folder for packaging and future US-specific edits. Use when the input text is English (or when no `--locale` / `HUMANIZER_LOCALE` is set).
