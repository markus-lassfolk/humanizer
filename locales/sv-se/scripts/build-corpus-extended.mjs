 ...
 * - Writes to locales/sv-se/tests/fixtures/sv-corpus-extended/ (gitignored)
 *
 * Usage: node locales/sv-se/scripts/build-corpus-extended.mjs
 * Env:
...
const outDir = path.join(REPO_ROOT, 'locales/sv-se/tests/fixtures/sv-corpus-extended');
...
async function main() {
  ...
  try {
    ...
    console.log(`Wrote ${i} article extracts to ${path.relative(REPO_ROOT, outDir)}`);
  } catch (e) {
