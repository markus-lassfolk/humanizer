...
- **Empirical data** — bundled `locales/sv-se/references/sv-frequencies.json` + `locales/sv-se/references/empirical-sv-tiers.md`: (1) **weights** on curated tier words when keys match, (2) **`empiricalExtra`** multi-word n-grams scored automatically in Pattern 7 (excludes unigrams; see `src/locales/sv-empirical-filter.js`). Log-odds can include **4-grams** and optional `locales/sv-se/tests/fixtures/sv-corpus/ai-llm/`. Maintainers: **`npm run sv:pipeline`** regenerates everything and updates `references/PIPELINE-SNAPSHOT.md`; or run `npm run corpus:logodds` / `corpus:refresh` alone.
